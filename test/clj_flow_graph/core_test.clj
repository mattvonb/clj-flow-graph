...
(deftest max-flow-potentially-pathological
  (testing "If our algorithm is... unlucky, this may time out."
    (let [graph (g/flow-graph [:s :a 50000] [:s :c 50000]
                         [:a :b 1]     [:b :c 1]
                         [:a :t 50000] [:c :t 50000])
          flow (max-flow graph :s :t)]
      (is (= flow 100000)))))
...
(deftest max-flow-good-example
  (testing "Just an example I like."
    (let [graph (g/flow-graph [:s :a 5] [:s :b 7] [:s :c 6]
                         [:a :d 4] [:a :e 3] [:b :e 4]
                         [:b :f 1] [:c :f 5] [:d :t 3]
                         [:e :t 7] [:f :e 4] [:f :t 6])
          flow (max-flow graph :s :t)]
